...
func main() {
	...
	opts := append(chromedp.DefaultExecAllocatorOptions[:],
		...
		chromedp.Flag("window-size", "1920,1050"),
	...
	cmd := exec.Command("ffmpeg",
		"-y",
		"-f", "image2pipe",
		"-r", fmt.Sprintf("%d", 60), // todo 快手的页面视频是60帧的
		"-i", "-",
		"-c:v", "libx264",
		"-preset", "slow",
		"-crf", "5",
	...
	log.Printf("run 1")
	err = chromedp.Run(ctx, chromedp.Tasks{
		chromedp.Navigate("https://www.kuaishou.com/short-video/3xz5b9cicvt86qs"),
		// 等待播放按钮出现
		chromedp.WaitVisible(`//*[@id="app"]//div[@class='kwai-player-container-video']/video`),
		// 等待播放按钮
		chromedp.WaitVisible(`//*[@id="app"]//div[@class='pause-icon']`),
		// 点击播放按钮
